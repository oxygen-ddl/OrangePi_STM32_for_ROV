                                                ...
                                                int timeout_ms)
{
    ...
    auto h = comm_gcs::codec::make_header(
        static_cast<std::uint8_t>(rovctrl::io::gcs::MsgType::CONNECT_REQ),
        seq++,
        0,
        rovctrl::io::gcs::FLAG_ACK_REQ,
    ...
    while (!g_stop.load()) {
        ...
        if (mt != rovctrl::io::gcs::MsgType::CONNECT_ACK) continue;

        if (!comm_gcs::codec::payload_size_is(pp->payload, sizeof(rovctrl::io::gcs::ConnectAck))) {
            continue;
        }
        ...
        auto h2 = comm_gcs::codec::make_header(
            static_cast<std::uint8_t>(rovctrl::io::gcs::MsgType::CONNECT_CONFIRM),
            seq++,
            r.session_id,
            rovctrl::io::gcs::FLAG_ACK_REQ,
    ...
    while (!g_stop.load()) {
        ...
        if (mt == rovctrl::io::gcs::MsgType::ACK) {
            if (comm_gcs::codec::payload_size_is(pp->payload, sizeof(rovctrl::io::gcs::AckPayload))) {
                ...
                std::cout << "[ACK] ack_seq=" << ap.ack_seq
                          << " code=" << ap.ack_code << "\n";
            }
