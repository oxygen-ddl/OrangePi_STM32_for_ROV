...
/**
 * @brief 控制模式请求（仅表达“操作员想切换”，是否允许由上层决定）
 *
 * 注意：此枚举应与 C++ 的 rovctrl::io::ControlMode 保持一致（编号也建议一致），
 * 但这里是 C ABI：只定义最小集合即可。
 */
typedef enum {
    PWM_TELEOP_MODE_NONE = 0,
    PWM_TELEOP_MODE_MANUAL,
    PWM_TELEOP_MODE_HOLD_ATT,
    PWM_TELEOP_MODE_HOLD_DEPTH,
    PWM_TELEOP_MODE_VELOCITY,
    PWM_TELEOP_MODE_POSITION,
    PWM_TELEOP_MODE_MPC
} pwm_teleop_mode_t;

/**
 * @brief teleop 扩展事件 bitmask
 */
typedef enum {
    PWM_TELEOP_EVT_NONE        = 0,
    PWM_TELEOP_EVT_MODE        = 1 << 0,  /**< 请求切换模式 */
    PWM_TELEOP_EVT_ARM         = 1 << 1,  /**< 请求 ARM */
    PWM_TELEOP_EVT_DISARM      = 1 << 2,  /**< 请求 DISARM */
    PWM_TELEOP_EVT_ESTOP       = 1 << 3,  /**< 请求 E-STOP */
    PWM_TELEOP_EVT_REF_DELTA   = 1 << 4   /**< 参考量增量（例如 depth/yaw） */
} pwm_teleop_evt_mask_t;

/**
 * @brief 扩展事件结构体：一次按键可能触发多个事件
 *
 * 约定：
 * - ref_delta_* 表示“目标量增量”，由上层叠加到当前 setpoint
 * - 单位建议：
 *   * ref_delta_depth_m: 米（+ 上浮 / - 下潜）
 *   * ref_delta_yaw_rad: 弧度（+ 右转 / - 左转）
 */
typedef struct {
    uint32_t           evt_mask;           /**< pwm_teleop_evt_mask_t 的组合 */
    pwm_teleop_mode_t  mode;               /**< 当 EVT_MODE 置位时有效 */

    float ref_delta_depth_m;               /**< 当 EVT_REF_DELTA 置位时可用 */
    float ref_delta_yaw_rad;               /**< 当 EVT_REF_DELTA 置位时可用 */
} pwm_teleop_event_t;

/**
 * @brief 处理单个按键（扩展版）：在保持 DOF 档位功能的同时，输出事件
 *
 * 行为：
 * - 内部仍会更新 DOF 档位（等价于 pwm_teleop_handle_key）
 * - 若触发模式/安全/参考量增量事件，则写入 out_evt
 *
 * @param key      按键编码
 * @param out_evt  输出事件（可为 NULL；为 NULL 时仅更新 DOF）
 *
 * @return 同 pwm_teleop_handle_key()
 */
int pwm_teleop_handle_key_ex(int key, pwm_teleop_event_t* out_evt);
