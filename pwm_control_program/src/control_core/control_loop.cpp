...
int ControlLoop::run()
{
    ...
    while (true) {
        ...
        if (dt <= 0.0) {
            dt = 1.0 / loop_hz;  // 防守性兜底
        }
        last_tick = now;
        next_tick = now + loop_period;

        if (cfg_.log_timing) {
            // 这里可以根据需要打印 dt 或做统计，目前只是留接口
        ...
        //       thruster_command 视为已经完成 DOF→推进器映射的归一化指令，
        //       由 controller 或 allocation 层负责物理意义；
        //       ControlLoop 仅负责下发；
        //   - 如果将来只给 body_wrench（has_body_wrench = true），
        //       则应在 controller 内部或单独的 ThrustAllocator 中完成映射，
        //       ControlLoop 不做智能推力分配。
        //
        ...
        if (step_rc < 0) {
            ...
            step_err_count = 0;  // 只要有一次成功，就清零错误计数
        }
    }
