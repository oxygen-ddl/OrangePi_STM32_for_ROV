...
public:
    ...
    // NEW: reply using the same bound socket (source port == cfg_.local.port)
    bool send_to(const UdpAddress& to, BytesView payload, std::string* err = nullptr) const noexcept;
