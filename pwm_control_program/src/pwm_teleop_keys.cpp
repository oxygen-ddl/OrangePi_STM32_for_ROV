...
void pwm_teleop_print_help(void)
{
    std::cout <<
        ...
        "    A / D : 右移 / 左移       （调整 sway 档位）\n"
        "    Q / E : 左转 / 右转       （调整 yaw 档位）\n"
        "    -> 多次按键会叠加成不同组合：例如 W+A+Q = 前进+右移+左转\n"
        "\n"
        "  [垂向 DOF 控制]\n"
        "    H : 上浮（heave 档位增加）\n"
        "    G : 下潜（heave 档位减小）\n"
...
static int apply_cmds_to_pwm()
{
    ...
    // surge：+1 时等价于原来的 W（前进：CH3、CH4 正向）
    d3 += su;
    d4 += su;

    // sway：+1 时等价于原来的 A（右移：CH1、CH3 正向）
    d1 += sw;
    d3 += sw;

    // yaw：+1 时等价于原来的 Q（左转：CH1、CH4 正向）
    d1 += yw;
    d4 += yw;
...
int pwm_teleop_handle_key(int key)
{
    ...
    case 'W':  // 前进：surge 增加
        ...
        if (rc < 0) std::cerr << "[ERR] apply_cmds_to_pwm (W) rc=" << rc << "\n";
        else        std::cout << "[KEY] W -> surge=" << g_cmd_surge
                              << " sway="  << g_cmd_sway
                              << " yaw="   << g_cmd_yaw
                              << " heave=" << g_cmd_heave << "\n";
        return (rc < 0) ? rc : 1;
    ...
    case 'S':  // 后退：surge 减少
        g_cmd_surge = clamp_unit(g_cmd_surge - CMD_STEP);
        rc = apply_cmds_to_pwm();
        if (rc < 0) std::cerr << "[ERR] apply_cmds_to_pwm (S) rc=" << rc << "\n";
        else        std::cout << "[KEY] S -> surge=" << g_cmd_surge
                              << " sway="  << g_cmd_sway
                              << " yaw="   << g_cmd_yaw
                              << " heave=" << g_cmd_heave << "\n";
        return (rc < 0) ? rc : 1;

    case 'D':  // 右移：sway 增加（如果你更习惯 其他键位，只需调整一下符号）
        g_cmd_sway = clamp_unit(g_cmd_sway + CMD_STEP);
        rc = apply_cmds_to_pwm();
        if (rc < 0) std::cerr << "[ERR] apply_cmds_to_pwm (A) rc=" << rc << "\n";
        else        std::cout << "[KEY] A -> surge=" << g_cmd_surge
                              << " sway="  << g_cmd_sway
                              << " yaw="   << g_cmd_yaw
                              << " heave=" << g_cmd_heave << "\n";
        return (rc < 0) ? rc : 1;
    ...
    case 'A':  // 左移：sway 减少
        g_cmd_sway = clamp_unit(g_cmd_sway - CMD_STEP);
        rc = apply_cmds_to_pwm();
        if (rc < 0) std::cerr << "[ERR] apply_cmds_to_pwm (D) rc=" << rc << "\n";
        else        std::cout << "[KEY] D -> surge=" << g_cmd_surge
                              << " sway="  << g_cmd_sway
                              << " yaw="   << g_cmd_yaw
                              << " heave=" << g_cmd_heave << "\n";
        return (rc < 0) ? rc : 1;
    ...
    case 'Q':  // 左转：yaw 增加
        ...
        rc = apply_cmds_to_pwm();
        if (rc < 0) std::cerr << "[ERR] apply_cmds_to_pwm (Q) rc=" << rc << "\n";
        else        std::cout << "[KEY] Q -> surge=" << g_cmd_surge
                              << " sway="  << g_cmd_sway
                              << " yaw="   << g_cmd_yaw
                              << " heave=" << g_cmd_heave << "\n";
        return (rc < 0) ? rc : 1;
    ...
    case 'E':  // 右转：yaw 减少
        g_cmd_yaw = clamp_unit(g_cmd_yaw - CMD_STEP);
        rc = apply_cmds_to_pwm();
        if (rc < 0) std::cerr << "[ERR] apply_cmds_to_pwm (E) rc=" << rc << "\n";
        else        std::cout << "[KEY] E -> surge=" << g_cmd_surge
                              << " sway="  << g_cmd_sway
                              << " yaw="   << g_cmd_yaw
                              << " heave=" << g_cmd_heave << "\n";
        return (rc < 0) ? rc : 1;
    ...
    case 'H':  // 上浮：heave 增加
        ...
        rc = apply_cmds_to_pwm();
        if (rc < 0) std::cerr << "[ERR] apply_cmds_to_pwm (H) rc=" << rc << "\n";
        else        std::cout << "[KEY] H -> surge=" << g_cmd_surge
                              << " sway="  << g_cmd_sway
                              << " yaw="   << g_cmd_yaw
                              << " heave=" << g_cmd_heave << "\n";
        return (rc < 0) ? rc : 1;
    ...
    case 'G':  // 下潜：heave 减少
        g_cmd_heave = clamp_unit(g_cmd_heave - CMD_STEP);
        rc = apply_cmds_to_pwm();
        if (rc < 0) std::cerr << "[ERR] apply_cmds_to_pwm (G) rc=" << rc << "\n";
        else        std::cout << "[KEY] G -> surge=" << g_cmd_surge
                              << " sway="  << g_cmd_sway
                              << " yaw="   << g_cmd_yaw
                              << " heave=" << g_cmd_heave << "\n";
        return (rc < 0) ? rc : 1;
