...
bool PwmFrameBuilder::parseHeartbeatAckV1(string_view frame,
    ...
    const size_t crc_len = 10 + static_cast<size_t>(len);

    const uint16_t crc_calc =
    proto::Crc16Ccitt::compute(p + ver_offset, crc_len);

    const uint16_t crc_in =
    readU16BE(p + 2 + 10 + len);
...
PwmFrameBuilder::parseStatusV1(string_view frame) {
    ...
    const std::size_t crc_len    = static_cast<std::size_t>(10) + static_cast<std::size_t>(len);
    const uint16_t crc_calc = proto::Crc16Ccitt::compute(p + ver_offset, crc_len);
